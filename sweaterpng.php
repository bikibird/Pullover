...
$hightlight=$HTTP_GET_VARS['Highlight'];
...
if ($hightlight=="S")
	{
		$sleeveColor = ImageColorAllocate($image,0,0,0);
		$bodyColor = ImageColorAllocate($image,128,128,128);
	}	
else
	{
		$sleeveColor = ImageColorAllocate($image,128,128,128);
		$bodyColor = ImageColorAllocate($image,0,0,0);
	}
...
switch($neckline)
{
	...
	case "B":
		ImageLine($image,$centerX-$NeckW1/2*$scale,$topY+$scale,$centerX+($NeckW1)/2*$scale,$topY+$scale,$bodyColor);
		break;
	case "R":
		imagearc($image,$centerX,$topY+$scale,$NeckW1*$scale,$NeckD*2*$scale,0,180,$bodyColor);
		break;
	case "V":
		ImageLine($image,$centerX-$NeckW1/2*$scale,$topY+$scale,$centerX,$topY+$scale+$NeckD*$scale,$bodyColor);
		ImageLine($image,$centerX+$NeckW1/2*$scale,$topY+$scale,$centerX,$topY+$scale+$NeckD*$scale,$bodyColor);
		break;
		
	case "S":
		ImageLine($image,$centerX-$NeckW1/2*$scale,$topY+$scale,$centerX-$NeckW2/2*$scale,$topY+$scale+$NeckD*$scale,$bodyColor);
		ImageLine($image,$centerX+$NeckW1/2*$scale,$topY+$scale,$centerX+$NeckW2/2*$scale,$topY+$scale+$NeckD*$scale,$bodyColor);
		ImageLine($image,$centerX-$NeckW2/2*$scale,$topY+$scale+$NeckD*$scale,$centerX+($NeckW2)/2*$scale,$topY+$scale+$NeckD*$scale,$bodyColor);
		break;
...
ImageLine($image,$centerX-$NeckW1/2*$scale,$topY+$scale,$centerX-($ShW)/2*$scale,$topY+$scale+$ShS*$scale,$bodyColor);
ImageLine($image,$centerX+$NeckW1/2*$scale,$topY+$scale,$centerX+($ShW)/2*$scale,$topY+$scale+$ShS*$scale,$bodyColor);
...
ImageLine($image,$currXLeft,$currY,$currXLeft,$currY+$ShUArm*$scale,$bodyColor);
ImageLine($image,$currXRight,$currY,$currXRight,$currY+$ShUArm*$scale,$bodyColor);
$currY =  $currY+$ShUArm*$scale;

ImageLine($image,$currXLeft,$currY,$currXLeft-$AHW1*$scale,$currY+$AHD*$scale,$bodyColor);
ImageLine($image,$currXRight,$currY,$currXRight+$AHW1*$scale,$currY+$AHD*$scale,$bodyColor);

$currXLeft=$currXLeft-$AHW1*$scale;
$currXRight=$currXRight+$AHW1*$scale;
$currY=$currY+$AHD*$scale;

ImageLine($image,$currXLeft,$currY,$currXLeft-$AHW2*$scale,$currY,$bodyColor);
ImageLine($image,$currXRight,$currY,$currXRight+$AHW2*$scale,$currY,$bodyColor);
...
ImageLine($image,$currXLeft,$currY,$centerX-$WW/2*$scale,$currY+$AWL*$scale,$bodyColor);
ImageLine($image,$currXRight,$currY,$centerX+$WW/2*$scale,$currY+$AWL*$scale,$bodyColor);
...
ImageLine($image,$currXLeft,$currY,$centerX-$HW/2*$scale,$currY+$WHL*$scale,$bodyColor);
ImageLine($image,$currXRight,$currY,$centerX+$HW/2*$scale,$currY+$WHL*$scale,$bodyColor);
...
ImageLine($image,$currXLeft,$currY,$currXRight,$currY,$bodyColor);
	


	//Right Arm Cap

if ($SCD>0 and $sleeve=="S")
{
	$SB=$ShUArm*2;
	$C=$UAW-(($AHW2+$AHW1)*2);
	$H=$SCD;
	...
	$circleX=$centerX+($ShW/2)*$scale+$H*$scale;

	//imagearc($image,$circleX, $circleY,$R*2*$scale,$R*2*$scale,180-$alpha,180+$alpha,$sleeveColor);
	imagearc($image,$circleX, $circleY,$H*2*$scale,$C*$scale,90,270,$sleeveColor);
}
else
{
	imageline($image,$centerX+($ShW/2)*$scale+$SCD*$scale,$topY+$scale+$shoulderSlant*$scale-($UAW-(($AHW2+$AHW1)*2))/2*$scale,$centerX+($ShW/2)*$scale+$SCD*$scale,$topY+$scale+$shoulderSlant*$scale+($UAW-(($AHW2+$AHW1)*2))/2*$scale, $sleeveColor);
}
...
imageline($image, $currX,$currTopY,$currX+$AHD*$scale, $currTopY-$AHW1*$scale, $sleeveColor);
imageline($image, $currX,$currBottomY,$currX+$AHD*$scale, $currBottomY+$AHW1*$scale, $sleeveColor);

$currX=$currX+$AHD*$scale;
$currTopY=$currTopY-$AHW1*$scale;
$currBottomY=$currBottomY+$AHW1*$scale;

imageline($image, $currX,$currTopY,$currX, $currTopY-$AHW2*$scale, $sleeveColor);
imageline($image, $currX,$currBottomY,$currX, $currBottomY+$AHW2*$scale, $sleeveColor);

$currTopY=$currTopY-$AHW2*$scale;
$currBottomY=$currBottomY+$AHW2*$scale;
$currY=$topY+$scale+$shoulderSlant*$scale;

imageline($image, $currX,$currY+$UAW/2*$scale,$currX+$UAL*$scale, $currY+$MAW/2*$scale, $sleeveColor);
imageline($image, $currX,$currY-$UAW/2*$scale,$currX+$UAL*$scale, $currY-$MAW/2*$scale, $sleeveColor);

$currX=$currX+$UAL*$scale;

imageline($image, $currX,$currY+$MAW/2*$scale,$currX+$LAL*$scale, $currY+$LAW/2*$scale, $sleeveColor);
imageline($image, $currX,$currY-$MAW/2*$scale,$currX+$LAL*$scale, $currY-$LAW/2*$scale, $sleeveColor);

$currX=$currX+$LAL*$scale;

imageline($image, $currX, $currY+$LAW/2*$scale, $currX, $currY-$LAW/2*$scale,$sleeveColor);
